...
const baseMatrix: MvpModuleResult[] = [
  ...
  module('账号与平台', allCriteria, ['账号、入口、API 配置、落地页、线索转入北森、同步日志', '已补账号健康主流程测试'], ['真实平台 API 字段仍需由使用人配置后复验']),
  ...
  module('AI工作台', allCriteria, ['模型配置、默认模板、业务上下文、试跑、日志、失败重试', '已补模型配置主流程测试'], ['真实模型 API 连通性需按现场密钥复验']),
