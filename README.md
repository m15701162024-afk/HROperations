...
当前版本：核心闭环版。主界面已从系统模块式导航重构为招聘内容运营工作台，只保留运营首页、内容工厂、渠道数据、连接配置四个入口。

## 功能范围

- 运营首页：从后端 `/api/ops/summary` 或本地数据聚合岗位、内容、发布、数据回收和渠道状态
- 内容工厂：岗位内容简报、AI 内容生成、风险扫描、审核意见、发布检查、选题和排期
- 渠道数据：平台指标导入、渠道下钻、点击/转化分析、复盘建议生成
- 连接配置：平台 API 集成、真实账号同步、北森/企业微信/飞书连接、大模型 API 配置
