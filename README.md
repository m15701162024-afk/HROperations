...
招聘运营助手是一个面向招聘运营专员的内容运营工具，核心目标是减少手工判断和跨模块跳转，围绕“岗位简报 -> 渠道内容 -> 发布排期 -> 渠道数据复盘”的闭环工作流展开。

当前版本：核心闭环版。主界面已从系统模块式导航重构为招聘内容运营工作台，只保留运营首页、内容工厂、渠道数据、连接配置四个一级入口；岗位、线索、复盘、落地页、系统配置等能力作为二级页签归入对应模块。

## 功能范围

- 运营首页：从后端 `/api/ops/summary` 或本地数据聚合岗位、内容、发布、数据回收和渠道状态
- 内容工厂：岗位内容简报、岗位库、AI 内容生成、风险扫描、审核意见、发布检查、选题和排期
- 渠道数据：平台指标导入、渠道下钻、点击/转化分析、线索池、北森回流归因、复盘建议生成
- 连接配置：平台 API 集成、真实账号同步、账号健康度、招聘入口、招聘落地页、北森/企业微信/飞书连接、大模型 API 和系统配置
- 公开落地页：发布后可通过 `/landing/:slug` 访问候选人页面并提交联系方式
- 后端兼容：保留数据存储、鉴权、平台同步、指标导入、落地页和分析下钻接口
...
- 运营摘要：`GET /api/ops/summary` 返回运营首页所需的岗位、内容生产、发布、指标回收和渠道状态。
- 字段映射：平台 API 扩展配置支持 `method`、`endpointPath`、`resultPath`、`fieldMapping`。
- 健康与备份：健康检查会返回数据文件大小、备份数量和核心数据计数；备份接口会把数据文件复制到 `data/backups/`。
